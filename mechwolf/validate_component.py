...
def validate_component(component, warnings=True):
    ...
    for k, v in component.base_state().items():
        if not hasattr(component, k):
            if warnings:
                print(term.red(f"Invalid attribute {k} for {component}. Valid attributes are {component.__dict__}"))
            return False
        if isinstance(component.__dict__[k], ureg.Quantity) and ureg.parse_expression(v).dimensionality != component.__dict__[k].dimensionality:
            if warnings:
                print(term.red(f"Invalid dimensionality {ureg.parse_expression(v).dimensionality} for {k} for {component}."))
            return False
        elif not isinstance(component.__dict__[k], ureg.Quantity) and not isinstance(component.__dict__[k], type(v)):
            if warnings:
                print(term.red(f"Bad type matching for {k} in base_state dict. Should be {type(component.__dict__[k])} but is {type(v)}."))
            return False

    # ensure type of config is valid
    if not isinstance(component.config(), dict):
        if warnings:
            print(term.red(f"Must return dictionary for config method for {component}."))
        return False
    ...
    for k, v in component.config().items():
        ...
        if type(v) not in [tuple, list] or len(v) != 2 or not isinstance(v[0], type):
            if warnings:
                print(term.red(f"Invalid configuration for {k} in {component}. Should be (type, default)."))
            return False

    if isinstance(component, Sensor):
        if not callable(getattr(component, "read", None)):
            if warnings:
                print(term.red("Sensors must have a read method that returns the sensor's data"))
            return False
