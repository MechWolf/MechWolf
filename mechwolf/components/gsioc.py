...
class GsiocComponent:
    ...
    def connect(self):
        ...
        raise RuntimeError('Device with ID {} did not respond.'.format(self.gsioc_id - 128))
    ...
    def buffered_command(self, command):
        ...
        for char in command + '\r':
            ...
            if len(char) < 1 or char != byte:
                raise RuntimeError('Device did not respond to buffered command.'.format(self.gsioc_id - 128))
